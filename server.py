...
def add_color(tweets):
    ...
    for t in tweets:
        score = t['score']
        newscore = int(((score + 1) / 2) * 100)
        t['color'] = colors[newscore]
    return tweets
...
def tweets(name):
    ...
    fetch_result = fetch_tweets(api, name)
    ...
    medi = median(score_list)
    return render_template('tweets.html', tweets=tweets, median=medi, fetch_result=fetch_result, screenname = name)
...
def following(name):
    ...
    return render_template('following.html', users=users, name = name)
